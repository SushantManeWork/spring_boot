...
public class UserAddressService {
    ...
    public List<UserAddressDTOForDisplay> findAll() {
        final List<UserAddress> userAddresses = userAddressRepository.findAll(Sort.by("userAddressId"));
    ...
    public UserAddressDTOForDisplay get(final Integer userAddressId) {
        ...
        UserAddressDTOForDisplay userAddressDTO=userAddressRepository.findById(userAddressId).filter(userAddress -> userAddress.getDeletedBy()==null)
                .map(UserAddressDTOForDisplay::mapToDTO)
                .orElse(null);
    ...
    public UserAddressDTOForDisplay create(final UserAddressDTOForCreate userAddressDTO) {
        ...
        UserAddress userUserAddress=userAddressRepository.findByUserUserId(userAddressDTO.getUserId());
        ...
        UserAddressDTOForDisplay addressDTO=UserAddressDTOForDisplay.mapToDTO(userAddressRepository.save(userAddress));
        return addressDTO;
    }
    ...
    public UserAddressDTOForDisplay update(final Integer userAddressId, final UserAddressDTOForCreate userAddressDTO) {
        ...
        if (userAddressId<=0 ||userAddressDTO.getUserId()<=0 || userAddressDTO.getCity().isBlank()||userAddressDTO.getState().isBlank()
            ...
            exception.add("Invalid details");
        UserAddress userAddress = userAddressRepository.findById(userAddressId).filter(userAddress1 -> userAddress1.getDeletedBy()==null)
                .orElse(null);
        if (userAddress==null)
        ...
        return UserAddressDTOForDisplay.mapToDTO(userAddressRepository.save(userAddress));
    }
